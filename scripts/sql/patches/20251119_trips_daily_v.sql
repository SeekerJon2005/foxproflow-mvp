...
--   - покрытие маршрутом (сколько рейсов имеют route_km);
--   - плановая выручка, RPM/RPH на базе trips.price_rub_plan (0 — валидная цена).
...
SELECT
    -- ПЕРВЫЕ 8 КОЛОНОК: как в текущей схеме
    b.d                                            AS d,
    ...
    -- Средняя RPM по всем рейсам (где есть километраж)
    CASE
    ...
    -- Плановая RPM (по плановой выручке и километражу)
    CASE 
        WHEN SUM(COALESCE(b.route_km, 0)) > 0
        THEN SUM(b.price_rub_plan)
             / NULLIF(SUM(COALESCE(b.route_km, 0)), 0)
