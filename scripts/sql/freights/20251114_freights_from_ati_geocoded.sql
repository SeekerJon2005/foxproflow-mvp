-- 20251114_freights_from_ati_geocoded.sql
...
--   • основная версия заточена под ETL в public.freights и агентов citymap.
...
BEGIN
  ...
  IF NOT has_city THEN
    -------------------------------------------------------------------------
    -- Fallback-вариант без геоданных:
    --   • структура колонок такая же, как в основной версии;
    --   • все гео-поля NULL, чтобы не ломать ETL.
    -------------------------------------------------------------------------
  ...
  ELSE
    -------------------------------------------------------------------------
    -- 2) Нормальный вариант: JOIN на city_map по упрощённым ключам места
    --
    -- loading_place_key / unloading_place_key уже подготовлены в
    -- freights_from_ati_v (очищены/нормализованы) и используются:
    --   • в этом JOIN’е;
    --   • в agents.citymap.suggest для обучения city_map.
    -------------------------------------------------------------------------
    EXECUTE $view_main$
      CREATE OR REPLACE VIEW public.freights_from_ati_geocoded_v AS
      SELECT
  ...
  ---------------------------------------------------------------------------
  -- 3) Комментарии к витрине/колонкам (общие для обоих вариантов)
  ---------------------------------------------------------------------------
  EXECUTE $cmt$
